import { AnalysisRequest, AnalysisResponse, AnalysisResult, DiagramGenerationResponse, ExampleSummary, ExampleData, RefreshAnalysisRequest } from '@/types';
...
export async function refreshAnalysis(request: RefreshAnalysisRequest): Promise<AnalysisResponse> {
  const response = await fetch(`${API_BASE_URL}/analyze/refresh`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function fetchExamples(): Promise<ExampleSummary[]> {
  console.log('🔍 Fetching examples list...');
...
export async function fetchExampleData(exampleId: string): Promise<ExampleData> {
  console.log(`🔍 Fetching example data for ID: ${exampleId}`);
  ...
  console.log(`✅ Retrieved example data: ${exampleData.repo?.name || 'Unknown'}`);
  return exampleData;
} 