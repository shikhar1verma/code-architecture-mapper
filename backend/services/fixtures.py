...
class FixtureLoader:
    ...
    def _load_example_to_db(self, session: Session, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Check if example already exists (by name)
            existing_example = session.query(Example).filter_by(name=example_data['name']).first()
            
            if existing_example:
                logger.info(f"Example '{example_data['name']}' already exists, updating...")
                ...
                # Create new example
                logger.info(f"Creating new example: '{example_data['name']}'")
                example = Example(**example_data)
                session.add(example)
            
            # Flush to get the example ID
            session.flush()
            
            logger.info(f"Successfully loaded example '{example_data['name']}'")
            return True
